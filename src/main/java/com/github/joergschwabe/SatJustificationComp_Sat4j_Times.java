 ...
 */
public class SatJustificationComp_Sat4j_Times<C, I extends Inference<? extends C>, A>
		extends MinimalSubsetsFromProofs<C, I, A> {

	private static final SatJustificationComp_Sat4j_Times.Factory<?, ?, ?> FACTORY_ = new Factory<Object, Inference<?>, Object>();
	...
	private SatJustificationComp_Sat4j_Times(final Proof<? extends I> proof,
		...
		this.monitor = (TimeOutMonitor) monitor;
	}
	...
	private class Enumerator implements MinimalSubsetEnumerator<A>, Producer<Inference<? extends Integer>> {
		...
		public Enumerator(Object query) {
			this.query = query;
		}
		...
		private void compute() throws ContradictionException, TimeoutException, ParserException, IOException {
			...
			while (true) {
				...
				if (satClauseHandler_.isQueryDerivable(axiomSet)) {
					...
					listener_.newMinimalSubset(justification);
					monitor.stopJustComp();
				} else {
					monitor.startCycleComp();
	...
	private static class Factory<C, I extends Inference<? extends C>, A>
		...
		@Override
		public org.liveontologies.puli.pinpointing.MinimalSubsetEnumerator.Factory<C, A> create(
				Proof<? extends I> proof, InferenceJustifier<? super I, ? extends Set<? extends A>> justifier,
				InterruptMonitor monitor) {
			return new SatJustificationComp_Sat4j_Times<C, I, A>(proof, justifier, monitor);
		}
	}
